    ...
    <div className="relative group w-full mb-16" data-aos="fade-up">
      ...
      <div className="relative overflow-hidden rounded-[2.5rem] bg-[#0f0f12] border border-white/5 p-8 md:p-12 transition-all duration-700 hover:border-blue-500/30">
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-7 flex flex-col h-full justify-between">
            ...
            <div className="mt-10 pt-8 border-t border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
              <div className="flex flex-wrap gap-2 max-w-md">
                {exp.tags.map((tag) => (
                  <span key={tag} className="px-3 py-1.5 rounded-lg bg-[#1a1a1e] border border-white/5 text-[10px] font-bold text-gray-400 uppercase group-hover:bg-blue-500/10 group-hover:text-blue-300 transition-all">
                    {tag}
                  </span>
                ))}
              </div>
              ...
              >
                <span className="relative z-10 text-sm tracking-wider">VIEW CASE STUDY</span>
          ...
          {/* Right Side: Interactive Media Hint / Preview (5 cols) */}
          <div 
            onClick={() => navigate(`/case-study/${exp.id}`)}
            className="lg:col-span-5 relative w-full h-64 lg:h-80 rounded-2xl overflow-hidden bg-[#16161a] border border-white/5 group-hover:border-blue-500/20 transition-all duration-500 cursor-pointer shadow-inner group/media"
          >
            ...
            {exp.image ? (
              <img 
                ...
                className="w-full h-full object-cover opacity-40 group-hover/media:scale-110 group-hover/media:opacity-70 transition-all duration-700 ease-out"
              />
            ) : (
              // Geometric mesh fallback if no image is passed yet
              <div className="absolute inset-0 bg-gradient-to-br from-blue-500/10 to-transparent opacity-30 group-hover/media:scale-110 transition-transform duration-700" />
            )}

            {/* Media Overlay Badges: Directly signals video/images live inside */}
            <div className="absolute top-4 left-4 flex gap-2">
             {/* Change HiPhotoIcon to HiPhoto */}
<span className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-black/60 backdrop-blur-md border border-white/10 text-[10px] font-bold tracking-wider text-white">
  <HiPhoto className="text-blue-400 text-sm" /> IMAGES
</span>
              <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-black/60 backdrop-blur-md border border-white/10 text-[10px] font-bold tracking-wider text-white">
                <HiPlay className="text-emerald-400 text-sm animate-pulse" /> VIDEO DEMO
              </span>
            </div>
